...
class ComiteController extends Controller
{
    ...
    {
        $comites = Comite::join('convocatorias','convocatorias.comite_id','=','comites.id')->where('convocatorias.id','=',$id)->select('convocatorias.*','comites.*')->whereNULL('comites.deleted_at')->get();

         $convocatoria_id=$id;

         $estatus = Asistencia::where('miembro_id',Auth::user()->id)->where('convocatoria_id','=',$id)->whereNULL('deleted_at')->value('estatus');

         $noticias = Blog::join('categorias','categorias.id','=','blogs.categoria_id')->select(array('blogs.*','categorias.nombrecat'))->where('categorias.nombrecat','Noticias')->orderBy('blogs.created_at','DESC')->whereNULL('blogs.deleted_at')->take(1)->get();

        $infografias = Blog::join('categorias','categorias.id','=','blogs.categoria_id')->select(array('blogs.*','categorias.nombrecat'))->where('categorias.nombrecat','Infografias')->orderBy('blogs.created_at','DESC')->whereNULL('blogs.deleted_at')->take(1)->get();

        $capsulas = Blog::join('categorias','categorias.id','=','blogs.categoria_id')->select(array('blogs.*','categorias.nombrecat'))->where('categorias.nombrecat','Capsulas de video')->orderBy('blogs.created_at','DESC')->whereNULL('blogs.deleted_at')->take(1)->get();

        $eventos = Evento::leftJoin('patrocinadores','patrocinadores.id','=','eventos.patrocinadores')->select(array('eventos.*','patrocinadores.nombrepatroc','patrocinadores.logopatroc'))->orderBy('eventos.fecha','DESC')->whereNULL('eventos.deleted_at')->take(1)->get();
        $comite_aviso= Comite_Aviso::join('employees','employees.id','=','comite_avisos.escritopor')->where('comite',$id)->select('comite_avisos.*','employees.name')->orderBy('created_at','DESC')->whereNULL('comite_avisos.deleted_at')->get();
        $asist = Asistencia::where('miembro_id',Auth::user()->id)->where('convocatoria_id','=',$id)->select('asistencias.*')->whereNULL('deleted_at')->get();
            


           foreach( $comite_aviso as $imagen){
            if( $imagen->archivos){
            $proper1 = $imagen->archivos;
             }

            
                $prop2 = str_replace('"', ' ', $proper1);
                             
                $uploads2 = json_decode($prop2);       
                     
                $uploads = Upload::whereIN('id',$uploads2)->select(array('uploads.*'))->orderBy('id','DESC')->get();
            }
         
            return view('comite-post',compact('comites','convocatoria_id','estatus','eventos','noticias','infografias','capsulas','comite_aviso','uploads','asist'));
    ...
    {
         $comites = Comite::where('comites.id','=',$id)->select('comites.*')->get();
         $convocatoria_id=$id;
         
         
            return view('comite-miembros',compact('comites','convocatoria_id'));
